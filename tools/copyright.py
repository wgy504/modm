...
def get_git_history_authors(filename, m=0.5):
    ...
    command = "git log --follow --format=\"%h %aN %aI %s\" -M{}% -- {}".format(int(m * 100), filename)
    output = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout.read().decode("utf-8")
    for line in output.splitlines():
        match = re.search("(?P<sha>[a-f0-9]{7}) (?P<author>.*?) (?P<year>[0-9]{4})-[0-9]{2}", line)
        sha, author, year = match.group("sha"), match.group("author"), match.group("year")
...
def replace_all_copyright_authors(sha_since):
    for filename in get_all_files_recursive(".", sha_since):
        ...
        if license not in ["BSD-3-Clause", "MPL-2.0"]:
            continue
